...
{
    public class VoxClassProxyWithTargetGenerator : BaseProxyGenerator
    {
        private readonly Type[] _additionalInterfacesToProxy;

        public VoxClassProxyWithTargetGenerator(ModuleScope scope, Type targetType) : base(scope, targetType)
        {
            CheckNotGenericTypeDefinition(targetType, "targetType");
            EnsureDoesNotImplementIProxyTargetAccessor(targetType, "targetType");
        }
        public VoxClassProxyWithTargetGenerator(ModuleScope scope, Type classToProxy, Type[] additionalInterfacesToProxy,
            ...
            : base(scope, classToProxy)
        {
            ...
            options.Initialize();
            ProxyGenerationOptions = options;
            _additionalInterfacesToProxy = TypeUtil.GetAllInterfaces(additionalInterfacesToProxy).ToArray();
        }
        ...
        protected virtual Type GenerateType(string name, Type[] interfaces, INamingScope namingScope)
        {
            ...
            // Collect methods
            var typeContributors = contributors as IList<ITypeContributor> ?? contributors.ToList();

            foreach (var contributor in typeContributors)
            {
        ...
        {
            var cacheKey = new CacheKey(targetType, targetType, _additionalInterfacesToProxy, ProxyGenerationOptions);
        ...
        {
            var methodsToSkip = new List<MethodInfo>();
            var proxyInstance = new ClassProxyInstanceContributor(targetType, methodsToSkip, _additionalInterfacesToProxy,
            ...
            if (ProxyGenerationOptions.HasMixins)
            {
                foreach (var mixinInterface in ProxyGenerationOptions.MixinData.MixinInterfaces)
                {
                    ...
                    {
                        // OK, so the target implements this interface. We now do one of two things:
                        if (_additionalInterfacesToProxy.Contains(mixinInterface) &&
            ...
            { Logger = Logger };
            // 3. then additional interfaces
            foreach (var @interface in _additionalInterfacesToProxy)
            {
                if (targetInterfaces.Contains(@interface))
                {
                    if (typeImplementerMapping.ContainsKey(@interface))
            ...
            {
                HandleExplicitlyPassedProxyTargetAccessor(targetInterfaces, _additionalInterfacesToProxy);
            }
        ...
        private FieldReference CustomTargetField(ClassEmitter emitter)
        {
            var targetField = emitter.CreateField("__custom", targetType);
            emitter.DefineCustomAttributeFor<XmlIgnoreAttribute>(targetField);
            return targetField;
        }
        private void EnsureDoesNotImplementIProxyTargetAccessor(Type type, string name)
        {
            ...
            var message =
                $"Target type for the proxy implements {typeof(IProxyTargetAccessor)} " + 
        ...
        {
            name = name.Replace("Castle.Proxies.", "Voxteneo.Proxies.").Replace("Proxy", Guid.NewGuid().ToString().Replace("-", ""));
            IEnumerable<ITypeContributor> contributors;
            ...
            var customTargetField = CustomTargetField(emitter);
            ...
            emitter.CreateField("___internalData", typeof(Dictionary<string, object>));
            GenerateConstructors(emitter, targetType, constructorArguments.ToArray());
            GenerateParameterlessConstructor(emitter, targetType, interceptorsField);
